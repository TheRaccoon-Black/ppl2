...
class Laporan extends CI_Controller
{
    ...
    public function tambahkeluar()
    {
        ...
        if (empty($post)) {
            $id_user = $this->session->userdata('id');
            // $kategori = $this->M_transaksi->get_kategori($id_user);
            $kategori = $this->M_transaksi->get_kategori_keluar($id_user);
            // var_dump($kategori);
            $relasi = array(
                "kategori" => $kategori
            );
            $this->load->view('templates/header');
            $this->load->view('templates/sidebar');
            $this->load->view("v_tambahTransaksiKeluar", $relasi);
            $this->load->view('templates/footer');
        } else {
            $id_user = $this->input->post("id_user");
            $id_kat = $this->input->post("kategori");
    ...
    public function tambahmasuk()
    {
        ...
        if (empty($post)) {
            $id_user = $this->session->userdata('id');
            // $kategori = $this->M_transaksi->get_kategori($id_user);
            $kategori = $this->M_transaksi->get_kategori_masuk($id_user);
            // var_dump($kategori);
            $relasi = array(
                "kategori" => $kategori
            );
            $this->load->view('templates/header');
            $this->load->view('templates/sidebar');
            $this->load->view("v_tambahTransaksiMasuk", $relasi);
            $this->load->view('templates/footer');
        } else {
            $id_user = $this->input->post("id_user");
            $id_kat = $this->input->post("kategori");
